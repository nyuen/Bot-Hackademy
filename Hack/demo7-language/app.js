    ...
    function (session, args, next) {
        ...
        var place = builder.EntityRecognizer.findEntity(args.entities, 'builtin.places.place_type');
				...
				if (place.resolution.value == "restaurants")
				{
					if(!cuisine)
					{
						session.send("OK. Tu souhaites un restaurant. Une pr�f�rence ?");
					}
					else
					{
						session.send("OK. Tu souhaites un restaurant %s. Je g�re !", cuisine.entity);
						if (time)
						{
								session.send('R�servation pour le %d/%d/%d', time.getMonth() + 1, time.getDate(), time.getFullYear());
						}
					}	
				}
				else
				{
					  session.send("D�sol�. Je ne r�serve que les restaurants moi !");
				}
